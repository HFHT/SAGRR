        ...
        if ($stmt->num_rows == 1) {
            ...
            if (checkbrute($user_id, $mysqli) == true) {
                ...
                if ($db_password == $password) {
                    ...
                    $now = time();
                    $mysqli->query("INSERT INTO login_attempts(user_id, ltime)
                                    VALUES ('$user_id', '$now')");
                    return false;
                }
            }
...
function checkbrute($user_id, $mysqli) {
    ...
    if ($stmt = $mysqli->prepare("SELECT ltime 
                             FROM login_attempts 
                             WHERE user_id = ? 
                            AND ltime > '$valid_attempts'")) {
...
function dbPrep ($arg,$form)
{
	$ap = filter_input(INPUT_POST, $arg, FILTER_SANITIZE_STRING);
	if ($ap == "") {$ap="NULL";} 
	else {
		switch ($form) {
			case "d":
				$ap = "'".date('Y-m-d', strtotime($ap))."'";
				break;
			case "s":
				$ap = "'".$ap."'";
				break;
