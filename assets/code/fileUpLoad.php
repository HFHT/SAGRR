...
//check user is logged in...
    ...
    $error = $filename = $filesize = $fileloc = $thumb_name = $masterW = $masterH = $thumbW = $thumbH = $masterSize = $thumbSize = '';
//    $fileElementName = 'image-upload';
	$fileElementName = 'file';
    $img_base_dir = "../../uploads/";
    define ("MAX_SIZE","4000"); //define a maxim size for the uploaded images
   // define the width and height for the thumbnail
   // note that theese dimmensions are considered the maximum dimmension and are not fixed,
   // because we have to keep the image ratio intact or it will be deformed was 350x150
   define ("WIDTH","128");
   define ("HEIGHT","128");
   ...
   $mimeType=$_FILES[$fileElementName]['type'];
    // if it is not empty
    if ($image)
    {
...
//            $image_name=time();
            $image_name=uniqid('FN');

            //the new name will be containing the full path where will be stored (images folder)
            $master_name= $img_base_dir . $image_name . '.' .$extension;
...
//				$masterW = $masterWH[0];
//				$masterH = $masterWH[1];
//				$masterSize = round((filesize($master_name)/1000), 0) . 'kb';

				if ($filetype == "image") {
					$size=getimagesize($_FILES[$fileElementName]['tmp_name']);
				  // the new thumbnail image will be placed in images/thumbs/ folder
				  $thumb_name= $img_base_dir . $image_name .'_350.'.$extension;
				  ...
				  $thumb=make_thumb($master_name,$thumb_name,WIDTH,HEIGHT);				  
					...
					$thumbW = $thumbWH[0];
					$thumbH = $thumbWH[1];
					$thumbSize = round((filesize($thumb_name)/1000), 0) . 'kb';
				}
            }}
        }
    }
	...
	$ret = array(
		"msdsSheet" => $master_name,
		"msdsOriginal" => $filename,
		"msdsPicture" => $thumb_name,
		"msdsFileType" => $filetype,
		"ProductCode" => "",
		"error" => $error
    );
